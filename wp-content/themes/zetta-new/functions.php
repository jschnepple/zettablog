...
if ( function_exists('register_sidebar') )
	...
	'name' => __( 'Zetta Sidebar', 'zetta' ),
	'id' => 'sidebar-page',
        'before_widget' => '<li id="%1$s" class="widget %2$s">',
        'after_widget' => '</li> <div class="horizontal-border"></div>',
        'before_title' => '<h2 class="widgettitle helv">',
        'after_title' => '</h2>',
    ));
/**
 ...
 * @since 2.8.0
 */
class Zetta_Tag_Cloud extends WP_Widget {

	public function __construct() {
		$widget_ops = array( 'description' => __( "Zetta tag cloud.") );
		parent::__construct('tag_cloud', __('Zetta Tag Cloud'), $widget_ops);
	}
 ...
 * @since 2.8.0
 */
class Zetta_Recent_Posts extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'widget_zetta_recent_entries', 'description' => __( "Your site&#8217;s most recent Posts.") );
		parent::__construct('recent-posts', __('Zetta Recent Posts'), $widget_ops);
		$this->alt_option_name = 'widget_zetta_recent_entries';
	...
	public function widget($args, $instance) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Zetta Recent Posts' );
...
?>
		...
		<?php while ( $r->have_posts() ) : $r->the_post(); ?>
			<li class="helv">
				<a class="helv" href="<?php the_permalink(); ?>"><?php get_the_title() ? the_title() : the_ID(); ?></a>
			<?php if ( $show_date ) : ?>
				<span class="post-date helv"><?php echo get_the_date(); ?></span>
			<?php endif; ?>
			</li>
		<?php endwhile; ?>
		</ol>
...
<?php
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['widget_zetta_recent_entries']) )
			delete_option('widget_zetta_recent_entries');
...
?>
		...
		<label for="<?php echo $this->get_field_id( 'show_date' ); ?>"><?php _e( 'Display post dates?' ); ?></label></p>
<?php
	}
}
...
function catch_that_image() {
  ...
  if(empty($first_img)) {
    $first_img = "http://pages.zetta.net/rs/zetta/images/blog-default.jpg";
  }
  return $first_img;
}

function new_excerpt_more( $more ) {
	return '...<br> <a class="more-link" href="'. get_permalink( get_the_ID() ) . '">' . __('Read More', 'zetta.net') . '</a>';
}
